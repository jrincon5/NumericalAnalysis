 ...
 */
public class PolynomialNewtonInterpolation extends Fragment implements View.OnClickListener{
    View rootView;
    EditText inpt_amount_points_newton_interpolation, inpt_point, valuesPoints[][];
    Button btn_generate_point, btn_point_replace;
    TableLayout table_points;
    int Points;
    double Point;
    List<String> ValuesDoublePoints = new ArrayList<String>();
    SolutionNewtonInterpolation solution=new SolutionNewtonInterpolation();

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.tab_polynomial_newton_interpolation, container, false);
        inpt_amount_points_newton_interpolation=(EditText) rootView.findViewById(R.id.inpt_newton_interpolation_amount_points);
        inpt_point=(EditText) rootView.findViewById(R.id.inpt_reduction_newton__interpolation_point);

        table_points=(TableLayout) rootView.findViewById(R.id.points_newton_interpolation);

        btn_generate_point=(Button) rootView.findViewById(R.id.button_Generate_Points_Newton_Inerpolation);
        btn_point_replace=(Button) rootView.findViewById(R.id.button_Calculate_Polynomial_Newton_Interpolation);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_Generate_Points_Newton_Inerpolation:
                try {
                    Points=Integer.parseInt(inpt_amount_points_newton_interpolation.getText().toString());
            ...
            case R.id.button_Calculate_Polynomial_Newton_Interpolation:
                try {
                    catchPoints(Points);
