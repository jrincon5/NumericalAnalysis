 ...
 */
public class PolynomialReductionSystems extends Fragment implements View.OnClickListener{
    View rootView;
    EditText inpt_amount_points_reduction_systems, inpt_point, valuesPoints[][];
    Button btn_generate_point, btn_point_replace;
    TableLayout table_points;
    int Points;
    double Point;
    static List<String> ValuesDoublePoints = new ArrayList<String>();
    SolutionReductionSystems solution=new SolutionReductionSystems();

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.tab_polynomial_reduction_systems, container, false);
        inpt_amount_points_reduction_systems=(EditText) rootView.findViewById(R.id.inpt_reduction_systems_amount_points);
        inpt_point=(EditText) rootView.findViewById(R.id.inpt_reduction_systems__point);

        table_points=(TableLayout) rootView.findViewById(R.id.points_reduction_systems);

        btn_generate_point=(Button) rootView.findViewById(R.id.button_Generate_Points_Reduction_Systems);
        btn_point_replace=(Button) rootView.findViewById(R.id.button_Calculate_Polynomial_Reduction_Systems);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_Generate_Points_Reduction_Systems:
                try {
                    Points=Integer.parseInt(inpt_amount_points_reduction_systems.getText().toString());
            ...
            case R.id.button_Calculate_Polynomial_Reduction_Systems:
                try {
                    catchPoints(Points);
