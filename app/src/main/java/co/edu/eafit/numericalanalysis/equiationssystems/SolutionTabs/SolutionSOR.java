 ...
 */
public class SolutionSOR extends Fragment implements View.OnClickListener{
    View rootView;
    ...
    static double matrixSolution[][], Delta, Tolerance, w=0;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.tab_solution_sor, container, false);
        txt_MatrixA=(TextView) rootView.findViewById(R.id.text_matrixA_SOR);
        txt_VectorB=(TextView) rootView.findViewById(R.id.text_vectorB_SOR);
        txt_MatrixT=(TextView) rootView.findViewById(R.id.text_matrixT_SOR);
        txt_VectorC=(TextView) rootView.findViewById(R.id.text_vectorC_SOR);
        txt_VectorX=(TextView) rootView.findViewById(R.id.text_vectorX_SOR);

        inpt_SOR_W=(EditText) rootView.findViewById(R.id.inpt_W_SOR);

        btn_showSolution=(Button) rootView.findViewById(R.id.button_show_solution_sor);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.button_show_solution_sor:
                try {
                    w = Double.parseDouble(inpt_SOR_W.getText().toString());
                    mA = buildMa();
                    vB = buildVb();
                    buildVinit();
                    SOR(Iterations, Tolerance, Delta);
    ...
    public void SOR(int iter, double toler, double delta){
        ...
        Matrix lProd = L.times(w);
        Matrix dDiff = D.minus(lProd);
        Matrix tNum = dDiff.inverse();

        Matrix dDen = D.times((1-w));
        Matrix uProd = U.times(w);
        Matrix denT = dDen.plus(uProd);

        T = tNum.times(denT);

        Matrix cD = tNum.times(w);
        C = cD.times(vectorB);
